                            ...
                            <div class="card-header border-0">
                                <div class="row align-items-center">
                                    <div class="col-8">
                                        ...
                                        @endif
                                        <h3 class="mb-0">@lang('labels.contrato_empresa')</h3>
                                    ...
                                    <div class="adicionar">
                                        <a href="{{ route('contratos.export') }}" class="btn btn-warning m-r-5" >@lang('labels.exportar')</a>
                                ...
                                <table id="tabela" class="table table-striped table-hover text-center">
                                    ...
                                    @foreach ($convencoes as $convencao)
                                        <tbody>
                                            ...
                                            <td>{{ $convencao->nome_fantasia }}</td>
                                            <td>{{ $convencao->convencao->cct }}</td>
                                            <td>{{ $convencao->convencao->sind_patronal }}</td>
                                            <td>{{ $convencao->convencao->sind_laboral }}</td>
                                            <td>{{ $convencao->convencao->abrang }}</td>
                                            <td>
                                                <a
                                                    href="{{ route('contratos.show', ['id' => base64_encode($convencao->id)]) }}">
                                                    <button type="button" class="btn btn-success"><i
                                                            class="bi bi-eye"></i></button></a>
                                                <a
                                                    href="{{ route('contratos.edit', ['id' => base64_encode($convencao->id)]) }}"><button
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <form action="{{ route('empresas.destroy', ['id' => base64_encode($convencao->id)]) }}"
                            method="post">
                            @csrf
                            @method('DELETE')
